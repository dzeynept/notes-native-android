...
public class NotesActivity extends AppCompatActivity implements View.OnClickListener, NoteInterface {
    ...
    public void deleteFromDB(String note, int id) {
        DataBaseHelper dataBaseHelper = new DataBaseHelper(getApplicationContext());
        SQLiteDatabase db = dataBaseHelper.getWritableDatabase();
        try {
    ...
    public int updateNoteDB(String note, int id) {
        ...
        try {
            ContentValues cv = new ContentValues();
            cv.put(DataBaseHelper.NOTES_COLUMN_NOTES, note);
            int count = db.update(DataBaseHelper.NOTES_TABLE_NAME, cv, DataBaseHelper.NOTES_COLUMN_ID+" = ?",new String[]{String.valueOf(id)});
            return  count;
            startActivity(new Intent(NotesActivity.this, NotesActivity.class));
            finish();
        } catch (Exception e) {
    ...
    @Override
    public void showUpdateNotePopup(int position) {
