...
public class DataBaseHelper extends SQLiteOpenHelper {
    ...
    public static final String DATABASE_NAME = "notesdatabase3";
    ...
    public int updateNote(UserModel user) {
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(NOTES_USER_ID_COLUMN_NOTES, user.getUser_id_from_notes());
        values.put(NOTES_COLUMN_NOTES, user.getNote());
        return db.update(NOTES_TABLE_NAME, values, NOTES_COLUMN_ID + " = ?",
                new String[]{String.valueOf(user.getNote_uniq_id())});
    }
